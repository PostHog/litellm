...
@pytest.mark.asyncio
async def test_latency_uses_wall_clock_duration():
    """$ai_latency should be wall-clock (endTime - startTime), not response_time."""
    posthog_logger = PostHogLogger()
    standard_payload = create_standard_logging_payload()
    # Simulate a streamed call where response_time collapsed to TTFT (~0)
    standard_payload["stream"] = True
    standard_payload["startTime"] = 1000.0
    standard_payload["endTime"] = 1005.0
    standard_payload["completionStartTime"] = 1000.0002
    standard_payload["response_time"] = 0.0002  # TTFT, not wall-clock

    kwargs = {"standard_logging_object": standard_payload}
    event_payload = posthog_logger.create_posthog_event_payload(kwargs)

    assert event_payload["properties"]["$ai_latency"] == 5.0


@pytest.mark.asyncio
async def test_latency_falls_back_to_response_time_when_timestamps_missing():
    """If startTime/endTime are unavailable, fall back to response_time."""
    posthog_logger = PostHogLogger()
    standard_payload = create_standard_logging_payload()
    del standard_payload["startTime"]
    del standard_payload["endTime"]
    standard_payload["response_time"] = 3.3

    kwargs = {"standard_logging_object": standard_payload}
    event_payload = posthog_logger.create_posthog_event_payload(kwargs)

    assert event_payload["properties"]["$ai_latency"] == 3.3


@pytest.mark.asyncio
async def test_generation_id_is_populated():
    """$ai_generation_id should be populated from the payload id."""
    posthog_logger = PostHogLogger()
    standard_payload = create_standard_logging_payload()
    ...
    assert event_payload["properties"]["$ai_generation_id"] == "test_id"
    assert event_payload["properties"]["$ai_span_id"] == "test_id"


@pytest.mark.asyncio
async def test_bedrock_model_name_normalized():
    """Bedrock model strings should be normalized for $ai_model."""
    posthog_logger = PostHogLogger()

    cases = {
        "bedrock/us.anthropic.claude-sonnet-4-6-v1:0": "anthropic.claude-sonnet-4-6",
        "bedrock/eu.anthropic.claude-sonnet-4-6": "anthropic.claude-sonnet-4-6",
        "us.anthropic.claude-sonnet-4-6-v1:0": "anthropic.claude-sonnet-4-6",
    }
    for raw_model, expected in cases.items():
        standard_payload = create_standard_logging_payload()
        standard_payload["custom_llm_provider"] = "bedrock"
        standard_payload["model"] = raw_model

        kwargs = {"standard_logging_object": standard_payload}
        event_payload = posthog_logger.create_posthog_event_payload(kwargs)

        assert event_payload["properties"]["$ai_model"] == expected


@pytest.mark.asyncio
async def test_non_bedrock_model_name_unchanged():
    """Non-bedrock model names should be passed through untouched."""
    posthog_logger = PostHogLogger()
    standard_payload = create_standard_logging_payload()
    standard_payload["custom_llm_provider"] = "openai"
    standard_payload["model"] = "gpt-3.5-turbo"
