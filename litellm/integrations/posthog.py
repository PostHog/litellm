...
class PostHogLogger(CustomBatchLogger):
    ...
    def _create_posthog_properties(
        ...
        provider = self._safe_get(standard_logging_object, "custom_llm_provider", "")
        properties["$ai_model"] = self._normalize_model_name(
            self._safe_get(standard_logging_object, "model", ""), provider
        )
        properties["$ai_provider"] = provider
        ...
        # Latency: use wall-clock duration (endTime - startTime) rather than the
        # `response_time` field. For streamed responses `response_time` is the
        # time-to-first-token (completion_start_time - start_time), which collapses
        # to ~0 for providers (e.g. Bedrock Converse) that emit their first
        # event-stream frame almost immediately - masking the true generation time.
        properties["$ai_latency"] = self._get_wall_clock_latency(
    ...
    def _add_trace_properties(self, properties: Dict[str, Any], kwargs: Dict[str, Any]):
        ...
        properties["$ai_generation_id"] = span_id
    ...
    def _get_wall_clock_latency(
        self, standard_logging_object: StandardLoggingPayload
    ) -> float:
        """
        Wall-clock duration of the LLM call (`endTime - startTime`).

        Falls back to the precomputed `response_time` when the timestamps are
        unavailable. Note `response_time` is time-to-first-token for streamed
        calls, so the wall-clock computation is preferred whenever possible.
        """
        start_time = self._safe_get(standard_logging_object, "startTime")
        end_time = self._safe_get(standard_logging_object, "endTime")
        if isinstance(start_time, (int, float)) and isinstance(end_time, (int, float)):
            latency = end_time - start_time
            if latency >= 0:
                return latency
        return self._safe_get(standard_logging_object, "response_time", 0.0)

    def _normalize_model_name(self, model: Any, provider: Any) -> Any:
        """
        Normalize the model name used for `$ai_model`.

        For Bedrock, the raw model string carries a cross-region inference prefix
        (e.g. `us.`/`eu.`) and a version suffix (e.g. `-v1:0`), which fragments the
        `$ai_model` dimension across otherwise-identical models. Strip both so that
        e.g. `bedrock/us.anthropic.claude-sonnet-4-6-v1:0` and
        `bedrock/eu.anthropic.claude-sonnet-4-6` collapse to a single value.
        """
        ...
        try:
            ...
            # Strips the bedrock/ route prefix, ARN wrapping and the cross-region
            # inference prefix (us./eu./apac./jp./global.).
            base_model = BedrockModelInfo.get_base_model(model)
        except Exception:
            return model

        # Strip the trailing Bedrock version suffix, e.g. `-v1:0` or `:0`, so that
        # versioned and unversioned references to the same model don't fragment.
        import re

        return re.sub(r"(-v\d+)?:\d+$", "", base_model)
